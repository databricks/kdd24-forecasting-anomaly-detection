...
# https://github.com/eBay/RANSynCoders/blob/main/eval_methods.py
...
# https://github.com/zhhlee/InterFusion/blob/main/explib/eval_methods.py
...
def get_best_f1(score, label, threshold=None):
    ...
    if flag == 1:
        search_set.append((cur_min_anomaly_score, cur_anomaly_len, True))
    search_set.sort(key=lambda x: -x[0]) # NOTE: descend order; this assumes higher the score the more anomalous the event
    best_f1_res = - 1
    threshold = 1
    P = 0
    TP = 0
