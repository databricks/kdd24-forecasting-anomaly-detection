...
from utils.helpers import CONFIG_DIR, DIALECT, METRIC_TIME_COLUMN, TIMESTAMP_COLUMN, load_config
from utils.tables import BaseTable, DimensionTable, FactTable


class TimeGranularity(Enum):
    DAILY = f"date_trunc('day', {TIMESTAMP_COLUMN})"
    MONTHLY = f"date_trunc('month', {TIMESTAMP_COLUMN})"
    YEARLY = f"date_trunc('year', {TIMESTAMP_COLUMN})"
...
class Metric:
    ...
    def _apply_time_grain(self, query: exp.Select, time_grain: TimeGranularity):
        query = query.select(f"{time_grain.value} as {METRIC_TIME_COLUMN}")
    ...
    def _determine_source(self, q: QuerySpec, fact_tables: List[FactTable], dimension_tables: List[DimensionTable]):
    ...
    def get_query(self, q: QuerySpec, fact_tables: List[FactTable], dimension_tables: List[DimensionTable]):
        query = exp.Select()
        query = query.from_(self._determine_source(q, fact_tables, dimension_tables))
        query = query.where(f"{TIMESTAMP_COLUMN} between '{q.start_date}' and '{q.end_date}'")
        query = self._apply_time_grain(query, q.time_grain)
        query = query.select(self.conf.definition)
        query = self._apply_grouping(query, q.groupings)
        return query.sql(pretty=True, identify=True, dialect=DIALECT)

    @staticmethod
    def get(metric_name: str, metrics: List["Metric"]):
        return [metric for metric in metrics if metric.name() == metric_name].pop()
